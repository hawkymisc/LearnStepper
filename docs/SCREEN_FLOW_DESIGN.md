...
2026-07-21のハッカソン提出スコープでは、[`HACKATHON_SUBMISSION_DESIGN.md`](HACKATHON_SUBMISSION_DESIGN.md)
が実装対象の優先文書です。本書の診断、演習、最終評価、補習、教育課程準拠、全データ削除を含む画面モデルは
将来の完全版設計として保持し、focused MVPの残件とは数えません。focused MVPはS00A、ローカルプロフィール、
外部Codex CLIログイン確認、任意テーマ作成、保存済み目標・計画、学習対話、進捗、履歴、ノート、ブックマーク、設定、
アーカイブ、および範囲を明記した個別削除に限定します。

本件は単なる画面実装ではなく、学習開始、LLM の非同期応答、補習、進捗更新、オフライン復旧を横断する設計問題として扱います。

### focused MVPの観測可能な達成条件

ユーザーが、起動ごとの成人確認から、ローカルプロフィール、外部Codex CLI認証確認、任意テーマの作成、
保存済み目標・計画の確認、学習対話、進捗・履歴・ノート・ブックマークの閲覧、設定、アーカイブまで
迷わず移動できることです。成人確認前はRendererがHost Bridgeを解決せず、確認後に初めてローカル状態を
問い合わせます。Electronホストは成人確認後にのみsidecar、DB、外部CLI検出を開始します。
対象外テーマはスコープ告知のみで、キーワード拒否は行いません。
...
- MVPは18歳以上を対象とし、起動ごとに最初の画面で自己申告を確認します。確認結果は永続化しません。
- 成人確認前はプロフィール、プロジェクト、認証状態を読み込みません。これは本人確認・年齢認証ではありません。
- 医療・法律・金融テーマはMVPの対象外です。この文言は利用前のスコープ確認であり、入力キーワードの拒否や分類は行いません。
...
|---|---|---|---|---|
| S00A | 成人確認 | 18歳以上というMVP利用条件の自己申告 | 対象年齢、対象外テーマ、18歳以上として開始 | 未確認、確認済み |
| S00 | 起動・回復 | ローカルDB、App Server、前回処理の整合 | 起動段階、回復結果、再試行、保存済みデータのみ開く | 初期化中、回復中、利用可能、部分利用、起動失敗 |
| S01 | Codex認証状態 | 外部CLIの認証状態確認 | device-auth手順、ログイン状態を再確認 | CLI未検出、対応外、未認証、認証済み、失敗 |
...
```mermaid
flowchart TD
  Z[S00A 成人確認] -->|18歳以上として開始| A[S00 起動・回復]
  A -->|認証済み・プロフィールあり| D[S03 ダッシュボード]
  A -->|未認証| B[S01 外部Codex認証]
  A -->|ローカル閲覧のみ| D
...
flowchart TD
  ...
  A -->|Renderer再読込| E[保存済み履歴を表示]

  B -->|接続回復| F[状態整合]
  C -->|再認証成功| F
  D -->|再起動成功| F
  E -->|session.resume| H[リモートスレッド再開]
  H -->|conversation.reconcile| F
  H -->|失敗| I[履歴保持 + 入力無効 + 再試行]
  F -->|差異あり| I
  F -->|未完了ターンなし| A
  F -->|未完了ターンあり| G[応答中へ復帰 + 新規送信禁止 + 停止可能]
  G --> A
  I -->|再接続を試す| H
```
...
- 保存セッションでは、`session.resume` と `conversation.reconcile` が完了するまで新規送信を無効にします。
- Codex完了通知の項目IDと `thread/read` の位置別名は、同一ターン内の役割・種別・内容・順序が一致する候補が一意な場合だけ同一項目として扱います。候補0件または複数件は競合として停止します。
- provider内部のreasoning項目は整合性のためローカル保存しても、Renderer向けセッション、履歴、完了イベント本文には公開しません。`thread/read` からは再取込しません。
- 再照合後に未完了ターンが残る場合は、そのローカルTurn IDと状態を復元し、新規送信を無効にしたまま「回答を停止」を利用可能にします。
