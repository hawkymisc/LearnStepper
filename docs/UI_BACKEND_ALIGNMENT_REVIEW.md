...
|---|---|---|---|---|---|
| UI-BE-001 | S05〜S09、S14。`SetupFlow`、`TutorConversation`、`ExerciseScreen`、`SettingsScreen` | 根拠取得、AI診断、計画生成、ライブ対話、生成停止、問題生成・採点、全ローカルデータ削除が成功する体験になっています。 | Codex App ServerによるChatGPT login、会話transport、型付きRenderer契約は実装済みです。一方、根拠取得、AI診断・計画・演習、完全削除、教育的AI挙動はNIF-001〜005、NIF-007、NIF-009、NIF-023〜026で一部または全部が保留中です。 | 実装済みログインと会話だけを接続し、未完成画面はFocused MVPから除外します。 | プレビューではデータが保存されないことを明示します。接続済みIPCから成功確認を受けていない処理を、完了・保存・達成済みと表示しません。 |
...
| ID | 対象画面・実装箇所 | 現状 | バックエンド上の事実 | 画面プロトの修正案 | 確認条件 |
|---|---|---|---|---|---|
| UI-BE-008 | S00〜S03。`Screen`、初期表示、`Dashboard` | 起動直後に固定データ入りダッシュボードを表示し、初回プロフィール、初期化失敗、空状態がありません。 | `profile.get/update`、`project.list/get` とCodex認証状態取得は実装済みです。プロフィール未作成時は `NOT_FOUND` になり得ます。 | 起動ロード画面を追加し、初回プロフィール、空状態、通常ダッシュボードへ分岐します。未認証時はCodexログイン導線を表示します。 | 初期DB、プロフィールのみ、プロジェクトあり、読込失敗、未認証の状態を操作確認できます。 |
